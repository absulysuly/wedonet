...
export const FreelancerCard: React.FC<FreelancerCardProps> = ({ freelancer, onStartChat, onViewProfile }) => {
    const { t } = useLocale();
    return (
        <div className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-1 transition-transform duration-300 flex flex-col">
            <div className="p-6 flex-grow">
                <div className="flex items-center space-x-4 rtl:space-x-reverse">
                    <img className="h-20 w-20 rounded-full object-cover" src={freelancer.avatarUrl} alt={freelancer.name} />
                    <div>
                        ...
                        {freelancer.services?.[0] && <p className="text-amber-600 font-medium">{t(freelancer.services[0])}</p>}
                    </div>
                </div>
                <div className="mt-4">
                    <div className="flex items-center">
                    ...
                    <p className="text-gray-500 mt-2 text-sm">
                        <span className="font-semibold">{t('location')}:</span> {t(freelancer.location)}
                    </p>
                    <p className="text-gray-500 mt-1 text-sm">
                        <span className="font-semibold">{t('experience')}:</span> {freelancer.experienceYears} {t('years')}
                    </p>
                </div>
            </div>
            <div className="p-6 pt-0 mt-auto">
                 <div className="flex space-x-2 rtl:space-x-reverse">
                    <button 
                        ...
                        className="flex-1 bg-white border border-amber-600 text-amber-600 font-semibold py-2 rounded-md hover:bg-amber-50 transition-colors">
                        {t('viewProfile')}
                    </button>
                    <button 
                        onClick={() => onStartChat(freelancer)}
                        className="flex-1 bg-amber-600 text-white font-semibold py-2 rounded-md hover:bg-amber-700 transition-colors"
                    >
                        {t('message')}
                    </button>
                </div>
